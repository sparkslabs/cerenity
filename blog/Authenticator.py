...
class Authenticator(object):
   ...
   def remember(self,username):
      self.source.set_cookie("name",username,expires=int(time.time())+3600*24*365*30)

   def forget(self):
      self.source.set_cookie("name",self.notloggedin,expires=int(time.time())+3600*24*365*30)
