...
class Posts(object):
   ...
   def baseNode(self, nodeid): 
       if "." in nodeid: return nodeid[:nodeid.find(".")]
       return nodeid
   def isBase(self, nodeid): return not self.isReply(nodeid)
   def isReply(self, nodeid):
       if "." in nodeid: return True
       return False
   def replyDepth(self, comment): return comment["__NODEID__"].count(".")
   
   def replyNodeIDAsSortableID(self, nodeid): return [ int(y) for y in nodeid.split(".")] 
   def SortableIDAsNodeID(self, Snodeid): return (".".join([ str(x_) for x_ in Snodeid]) )
